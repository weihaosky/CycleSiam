...
def siamese_track(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    ...
    scale_x = p.exemplar_size / s_x     # template size reduce scale
    d_search = (p.instance_size - p.exemplar_size) / 2  # searching range after scale reduce
    pad = d_search / scale_x       # searching range before scale reduce
    s_x = s_x + 2 * pad     # Original total searching area (template + surrounding searching range)
    crop_box = [target_pos[0] - round(s_x) / 2, target_pos[1] - round(s_x) / 2, round(s_x), round(s_x)]     # searching area
    ...
    if mask_enable:
        ...
        if len(contours) != 0 and np.max(cnt_area) > 100:
            ...
            # prbox = np.array([[pbox[0], pbox[1]],
            #         [pbox[0] + pbox[2], pbox[1]],
            #         [pbox[0] + pbox[2], pbox[1] + pbox[3]],
            #         [pbox[0], pbox[1] + pbox[3]]])
...
def track_vot(model, video, hp=None, mask_enable=False, refine_enable=False, device='cpu'):
    ...
    for f, image_file in enumerate(image_files):
        ...
        if args.visualization and f >= start_frame:  # visualization (skip lost frame)
            ...
            if gt.shape[0] > f:
                if len(gt[f]) == 8:
                    cv2.polylines(im_show, [np.array(gt[f], np.int).reshape((-1, 1, 2))], True, (0, 255, 0), 2)
                else:
                    cv2.rectangle(im_show, (gt[f, 0], gt[f, 1]), (gt[f, 0] + gt[f, 2], gt[f, 1] + gt[f, 3]), (0, 255, 0), 2)
            if len(location) == 8:
                # if mask_enable:
                    # mask = mask > state['p'].seg_thr
                    # im_show[:, :, 2] = mask * 255 + (1 - mask) * im_show[:, :, 2]
                location_int = np.int0(location)
                cv2.polylines(im_show, [location_int.reshape((-1, 1, 2))], True, (0, 255, 255), 2)
            else:
                location = [int(l) for l in location]
                cv2.rectangle(im_show, (location[0], location[1]),
                              (location[0] + location[2], location[1] + location[3]), (255, 0, 255), 2)
            ...
            # cv2.waitKey(1)
            # print(f, image_file)
            img_path = 'visulization/' + video['name'] + '/'
            if not os.path.exists(img_path): 
                os.makedirs(img_path)
            cv2.imwrite(img_path + str(f) + '.jpg', im_show)
...
def MultiBatchIouMeter(thrs, outputs, targets, start=None, end=None):
    ...
    res = np.zeros((num_object, len(thrs), 2), dtype=np.float32)
        ...
        for j in range(num_object):
            ...
            for i in range(start_frame, end_frame):
                ...
                if union > 0:
                    ...
                    iou.append(1)
                contour_accu.append(db_eval_boundary(pred, target_j[i]))
            res[j, k, 0] = np.mean(iou)
            res[j, k, 1] = np.mean(contour_accu)
    return res
...
def track_vos(model, video, hp=None, mask_enable=False, refine_enable=False, mot_enable=False, device='cpu'):
    ...
    if len(annos) == len(image_files):
        ...
        for i in range(object_num):
            for j, thr in enumerate(thrs):
                logger.info('Fusion Multi Object{:20s} IOU at {:.2f}: {:.4f}, {:.4f} '.format(video['name'] + '_' + str(i + 1), thr,
                                                                           multi_mean_iou[i, j, 0], multi_mean_iou[i, j, 1]))
    else:
        multi_mean_iou = []
    ...
    if args.visualization:
        ...
        for f, image_file in enumerate(image_files):
            ...
            # if not os.path.exists(img_path): 
            #     os.makedirs(img_path)
            # cv2.imwrite(img_path + str(f) + '.jpg', output)
...
def main():
    ...
    if vos_enable:
        for thr, iou in zip(thrs, np.mean(np.concatenate(iou_lists), axis=0)):
            logger.info('Segmentation Threshold {:.2f} mIoU: {:.3f} mF: {:.3f}'.format(thr, iou[0], iou[1]))
    else:
