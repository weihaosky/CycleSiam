...
class SubDataSet(object):
    ...
    def get_positive_pair(self, index):
        ...
        if 'hard' not in track_info:
            frame1 = random.randint(0, len(frames)-1)
            left = max(frame1 - self.frame_range, 0)
            right = min(frame1 + self.frame_range, len(frames)-1) + 1
            frame2_range = frames[left:right]
            frame1 = frames[frame1]
            frame2 = random.choice(range(len(frame2_range)))

            left = max(frame2 - self.frame_range, 0)
            right = min(frame2 + self.frame_range, len(frames)-1) + 1
            frame3_range = frames[left:right]
            frame2 = frames[frame2]
            frame3 = random.choice(frame3_range)
        else:
            frame3 = random.choice(track_info['hard'])
            left = max(frame3 - self.frame_range, 0)
            right = min(frame3 + self.frame_range, len(frames)-1) + 1  # python [left:right+1) = [left:right]
            frame2_range = frames[left:right]
            frame2 = random.choice(range(len(frame2_range)))
            frame3 = frames[frame3]

            left = max(frame2 - self.frame_range, 0)
            right = min(frame2 + self.frame_range, len(frames)-1) + 1  # python [left:right+1) = [left:right]
            frame1_range = frames[left:right]
            frame2 = frames[frame2]
            frame1 = random.choice(frame1_range)

        return self.get_image_anno(video_name, track, frame1), \
               self.get_image_anno(video_name, track, frame2), \
               self.get_image_anno(video_name, track, frame3)
...
class DataSets(Dataset):
    ...
    def __getitem__(self, index, debug=False):
        ...
        if neg:
            img1 = dataset.get_random_target(index)
            if self.inner_neg and self.inner_neg > random.random():
                img2 = dataset.get_random_target()
                img3 = dataset.get_random_target()
            else:
                img2 = random.choice(self.all_data).get_random_target()
                img3 = random.choice(self.all_data).get_random_target()
        else:
            img1, img2, img3 = dataset.get_positive_pair(index)
        ...
        img1_image, scale1 = self.imread(img1[0])

        if self.template_small:
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("template_small")
            template_image = center_crop(template_image, self.template_size)

        img2_image, scale2 = self.imread(img2[0])
        img3_image, scale3 = self.imread(img3[0])

        if dataset.has_mask and not neg:
            img1_mask = (cv2.imread(img1[2], 0) > 0).astype(np.float32)
            img2_mask = (cv2.imread(img2[2], 0) > 0).astype(np.float32)
            img3_mask = (cv2.imread(img3[2], 0) > 0).astype(np.float32)
        else:
            img1_mask = np.zeros(img1_image.shape[:2], dtype=np.float32)
            img2_mask = np.zeros(img2_image.shape[:2], dtype=np.float32)
            img3_mask = np.zeros(img3_image.shape[:2], dtype=np.float32)
        ...
        img1_box = toBBox(img1_image, img1[1])
        img2_box = toBBox(img2_image, img2[1])
        img3_box = toBBox(img3_image, img3[1])

        img1_template, img1_bbox_t, _ = self.template_aug(img1_image, img1_box, self.template_size, gray=gray)
        img2_template, img2_bbox_t, _ = self.template_aug(img2_image, img2_box, self.template_size, gray=gray)
        img3_template, img3_bbox_t, _ = self.template_aug(img3_image, img3_box, self.template_size, gray=gray)
        # teamplate_image shape: (511, 511, 3)
        # self.template_size: 127
        # template shape: (127, 127, 3)
        img1_search, img1_bbox, img1_mask = self.search_aug(img1_image, img1_box, self.search_size, gray=gray, mask=img1_mask)
        img2_search, img2_bbox, img2_mask = self.search_aug(img2_image, img2_box, self.search_size, gray=gray, mask=img2_mask)
        img3_search, img3_bbox, img3_mask = self.search_aug(img3_image, img3_box, self.search_size, gray=gray, mask=img3_mask)
        # search_image shape: (511, 511, 3)
        # self.search_size: 255
        # search shape: (255, 255, 3)

        img1_cls, img1_delta, img1_delta_weight = self.anchor_target(self.anchors, img1_bbox, self.size, neg)
        img2_cls, img2_delta, img2_delta_weight = self.anchor_target(self.anchors, img2_bbox, self.size, neg)
        img3_cls, img3_delta, img3_delta_weight = self.anchor_target(self.anchors, img3_bbox, self.size, neg)
        if dataset.has_mask and not neg:
        # if False: #### train with only box 
            img1_mask_weight = img1_cls.max(axis=0, keepdims=True)
            img2_mask_weight = img2_cls.max(axis=0, keepdims=True)
            img3_mask_weight = img3_cls.max(axis=0, keepdims=True)
        else:
            img1_mask_weight = np.zeros([1, img1_cls.shape[1], img2_cls.shape[2]], dtype=np.float32)
            img2_mask_weight = np.zeros([1, img2_cls.shape[1], img2_cls.shape[2]], dtype=np.float32)
            img3_mask_weight = np.zeros([1, img3_cls.shape[1], img3_cls.shape[2]], dtype=np.float32)

        img1_template, img1_search, img2_template, img2_search, img3_template, img3_search = \
                        map(lambda x: np.transpose(x, (2, 0, 1)).astype(np.float32), \
                        [img1_template, img1_search, img2_template, img2_search, img3_template, img3_search])
        
        img1_mask = (np.expand_dims(img1_mask, axis=0) > 0.5) * 2 - 1  # 1*H*W
        img2_mask = (np.expand_dims(img2_mask, axis=0) > 0.5) * 2 - 1  # 1*H*W
        img3_mask = (np.expand_dims(img3_mask, axis=0) > 0.5) * 2 - 1  # 1*H*W

        return [img1_template, img2_search, img2_cls, img2_delta, img2_delta_weight, np.array(img1_bbox_t, np.float32), \
               np.array(img2_mask, np.float32), np.array(img2_mask_weight, np.float32)], \
               [img2_template, img1_search, img1_cls, img1_delta, img1_delta_weight, np.array(img2_bbox_t, np.float32), \
               np.array(img1_mask, np.float32), np.array(img1_mask_weight, np.float32)], \
               [img2_template, img3_search, img3_cls, img3_delta, img3_delta_weight, np.array(img2_bbox_t, np.float32), \
               np.array(img3_mask, np.float32), np.array(img3_mask_weight, np.float32)], \
               [img3_template, img2_search, img2_cls, img2_delta, img2_delta_weight, np.array(img3_bbox_t, np.float32), \
               np.array(img2_mask, np.float32), np.array(img3_mask_weight, np.float32)], \
